...
def calculate_new_metalobic_without_BMI(row):
    risk_score = (
        row['HighBP'] * 0.9789 + 
        row['HighChol'] * 0.958 + 
        row['HeartDiseaseorAttack'] * 0.80 +
        row['Stroke'] * 0.80
    )
...
brfss_binary['NewMetabolicDisorder'] = brfss_binary.apply(calculate_new_metalobic_without_BMI, axis=1)
...
# brfss_5050.to_csv('new_diabetes_binary_health_indicators_with_MD_BRFSS2023.csv', sep=",", index=False)
