...
def calculate_new_metalobic_multiplicative_without_BMI(row):
    # Multiplicative risk calculation without BMI
    risk_score = (
        (1 + row['HighBP'] * 0.9789) * 
...
brfss_binary['NewMetabolicDisorder'] = brfss_binary.apply(calculate_new_metalobic_multiplicative_without_BMI, axis=1)
...
# brfss_5050.to_csv('new_diabetes_binary_health_indicators_multiplicative_BRFSS2023.csv', sep=",", index=False)

brfss_5050['percentage_reduction'] = (
    (brfss_5050['MetabolicDisorderWithoutBMI'] - brfss_5050['NewMetabolicDisorder']) / brfss_5050['MetabolicDisorderWithoutBMI']
) * 100

# Calculate the average percentage reduction across all rows
average_reduction = brfss_5050['percentage_reduction'].mean()
print(f'Average Percentage Risk Reduction: {average_reduction:.2f}%')

average_reduction_diabetes = brfss_5050[brfss_5050['Diabetes_binary'] == 1]['percentage_reduction'].mean()
print(f'Average Percentage Risk Reduction for Diabetics: {average_reduction_diabetes:.2f}%')